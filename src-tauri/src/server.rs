...
struct ServerState {
    ...
    deploy_jobs: Arc<Mutex<HashMap<String, DeployJob>>>,
    active_deploy_job_id: Arc<Mutex<Option<String>>>,
...
struct UpdateSettingsPayload {
    ...
    hugo_project_path: Option<String>,
}
...
pub async fn start_server(app_handle: AppHandle) -> Result<ServerRuntime, String> {
    ...
    let state = Arc::new(ServerState {
        ...
        active_deploy_job_id: Arc::new(Mutex::new(None)),
    ...
    let router = Router::new()
        ...
        .route(
            "/api/editor/article",
            post(create_article).put(save_article),
        )
        .route("/api/editor/article/move", post(move_article))
        .route(
            "/api/editor/article/delete",
            post(delete_article).delete(delete_article),
        )
        ...
        .route("/api/editor/deploy", post(start_deploy))
        .route("/api/editor/deploy/:job_id", get(get_deploy_job))
...
async fn put_settings(
    ...
    let next_settings = EditorPathSettings {
        ...
        hugo_project_path: normalize_optional_path(payload.hugo_project_path.as_deref()),
    };

    let runtime_paths = resolve_runtime_content_paths(&next_settings, true)?;
    assert_directory_exists(
        &runtime_paths.document_project_path,
    ...
    assert_directory_exists(
        &runtime_paths.chain_code_repo_path,
        "Deploy repository path",
    )
    .await?;
    ...
    let normalized_hugo_project_path = if !next_settings.hugo_project_path.trim().is_empty() {
        let hugo_project_path = absolutize_path(&next_settings.hugo_project_path)?;
        assert_directory_exists(&hugo_project_path, "Hugo project path").await?;
        hugo_project_path.to_string_lossy().to_string()
    } else {
        String::new()
    };

    let persisted = EditorPathSettings {
        document_project_path: runtime_paths
            .document_project_path
            .to_string_lossy()
            .to_string(),
        chain_code_repo_path: runtime_paths
            .chain_code_repo_path
            .to_string_lossy()
            .to_string(),
        backup_root_path: normalized_backup_root_path,
        hugo_project_path: normalized_hugo_project_path,
    };
...
async fn get_content_tree(State(state): State<SharedState>) -> ApiResult {
    ...
    let root_section =
        tokio::task::spawn_blocking(move || build_section_sync(&docs_source_dir, ""))
            .await
            .map_err(|error| {
                ApiError::internal(format!("Failed to build content tree: {error}"))
            })??;
...
async fn get_article_markdown(
    ...
    let article_file =
        resolve_under_root(&runtime_paths.docs_source_dir, &normalized_article_path)?;
...
async fn move_article(
    ...
    let target_section_path =
        to_safe_relative_path(&payload.target_section_path, "targetSectionPath", true)?;
    let target_section_abs =
        resolve_under_root(&runtime_paths.docs_source_dir, &target_section_path)?;
    ...
    let target_article_abs =
        resolve_under_root(&runtime_paths.docs_source_dir, &target_article_path)?;
...
async fn delete_article(
    ...
    fs::remove_file(&article_abs).await.map_err(|error| {
        ApiError::internal(format!(
...
async fn start_deploy(State(state): State<SharedState>) -> ApiResult {
    let current_active = state.active_deploy_job_id.lock().await.clone();
    if let Some(active_id) = current_active {
        let is_running = {
            let jobs = state.deploy_jobs.lock().await;
            jobs.get(&active_id)
                .map(|job| job.status == "running")
                .unwrap_or(false)
        };
        ...
        let mut active_guard = state.active_deploy_job_id.lock().await;
        if active_guard.as_deref() == Some(active_id.as_str()) {
            *active_guard = None;
        }
    }

    let job_id = create_deploy_job(&state).await;
    *state.active_deploy_job_id.lock().await = Some(job_id.clone());

    let state_clone = state.clone();
    let job_id_clone = job_id.clone();
    tauri::async_runtime::spawn(async move {
        execute_deploy_job(state_clone, job_id_clone).await;
    });
...
async fn get_deploy_job(
    ...
    let job = {
        ...
        jobs.get(&job_id)
            .cloned()
            .ok_or_else(|| ApiError::not_found("Deploy job not found."))?
    };

    Ok(Json(to_deploy_job_response(&job)).into_response())
}
...
async fn execute_publish_job_inner(state: &SharedState, job_id: &str) -> Result<(), ApiError> {
    ...
    assert_git_repository(&repository_path, "内容仓库路径").await?;
    ...
    .await;
    let add_result =
        run_git_command_with_logs(state, job_id, &["add", "."], &repository_path).await;
    if !add_result.ok {
        return Err(ApiError::internal(format_git_command_error(
            "git add .",
            &add_result,
...
async fn execute_backup_job_inner(state: &SharedState, job_id: &str) -> Result<(), ApiError> {
    ...
    let scan_result =
        tokio::task::spawn_blocking(move || collect_backup_items(&source_repo_for_scan))
            .await
            .map_err(|error| {
                ApiError::internal(format!("Failed to scan repository files: {error}"))
            })??;
...
async fn create_deploy_job(state: &SharedState) -> String {
    let now = Utc::now().to_rfc3339();
    let sequence = state.publish_log_sequence.fetch_add(1, Ordering::Relaxed);
    let id = format!("deploy-{}-{sequence}", Utc::now().timestamp_millis());
    ...
    {
        let mut jobs = state.deploy_jobs.lock().await;
        jobs.insert(id.clone(), job);
    }
    append_deploy_log(state, &id, "info", "开始执行站点发布任务。").await;
    cleanup_deploy_jobs(state).await;
    id
}

async fn execute_deploy_job(state: SharedState, job_id: String) {
    let result = execute_deploy_job_inner(&state, &job_id).await;
    if let Err(error) = result {
        update_deploy_job(&state, &job_id, |job| {
            job.status = "error".to_string();
            job.stage = "done".to_string();
            job.message = Some(error.message.clone());
        })
        .await;
        append_deploy_log(&state, &job_id, "error", error.message).await;
    }
...
async fn execute_deploy_job_inner(state: &SharedState, job_id: &str) -> Result<(), ApiError> {
    let settings = state.runtime_settings.read().await.clone();
    let runtime_paths = resolve_runtime_content_paths(&settings, true)?;
    let hugo_project_path = resolve_hugo_project_path(&settings)?;
    let deploy_repo_path = runtime_paths.chain_code_repo_path;

    assert_directory_exists(&hugo_project_path, "Hugo project path").await?;
    assert_directory_exists(&deploy_repo_path, "Deploy repository path").await?;
    assert_git_repository(&deploy_repo_path, "部署仓库路径").await?;

    append_deploy_log(
        state,
        job_id,
        "info",
        format!("Hugo 目录：{}", hugo_project_path.display()),
    )
    .await;
    append_deploy_log(
        state,
        job_id,
        "info",
        format!("部署仓库：{}", deploy_repo_path.display()),
    )
    .await;

    update_deploy_job(state, job_id, |job| {
        job.stage = "hugo".to_string();
    })
    .await;
    let hugo_result =
        run_deploy_command_with_logs(state, job_id, "hugo", &[], &hugo_project_path).await;
    if !hugo_result.ok {
        return Err(ApiError::internal(format_git_command_error(
            "hugo",
            &hugo_result,
    ...
    .await;
    let add_result =
        run_deploy_command_with_logs(state, job_id, "git", &["add", "."], &deploy_repo_path).await;
    if !add_result.ok {
        return Err(ApiError::internal(format_git_command_error(
            "git add .",
            &add_result,
    ...
    let has_staged_changes = has_git_staged_changes(&deploy_repo_path).await?;
    let commit_message = format!("docs: update {}", get_current_date_label());
    if has_staged_changes {
        update_deploy_job(state, job_id, |job| {
            job.stage = "commit".to_string();
            job.commit_message = Some(commit_message.clone());
        ...
        .await;
        let commit_result = run_deploy_command_with_logs(
            state,
            job_id,
            "git",
            &["commit", "-m", &commit_message],
        ...
        append_deploy_log(state, job_id, "info", "未检测到暂存变更，跳过 git commit。").await;
    }
    ...
    .await;
    let push_result =
        run_deploy_command_with_logs(state, job_id, "git", &["push"], &deploy_repo_path).await;
    if !push_result.ok {
        return Err(ApiError::internal(format_git_command_error(
            "git push",
            &push_result,
    ...
    let success_message = if has_staged_changes {
        format!("发布完成：{commit_message}")
    } else {
        "发布完成：无改动需要提交，已执行 hugo 与 git push。".to_string()
    };
    update_deploy_job(state, job_id, |job| {
        job.status = "success".to_string();
        job.stage = "done".to_string();
    ...
    .await;
    append_deploy_log(state, job_id, "success", success_message).await;

    Ok(())
}

async fn append_deploy_log(
    state: &SharedState,
    job_id: &str,
    level: &str,
    text: impl Into<String>,
...
async fn update_deploy_job<F>(state: &SharedState, job_id: &str, updater: F)
where
...
async fn cleanup_deploy_jobs(state: &SharedState) {
    let mut jobs = state.deploy_jobs.lock().await;
    if jobs.len() <= 30 {
        return;
    }

    let mut sorted: Vec<DeployJob> = jobs.values().cloned().collect();
    sorted.sort_by(|left, right| left.started_at.cmp(&right.started_at));
    let removable: Vec<String> = sorted
        .iter()
        .filter(|job| job.status != "running")
...
fn to_deploy_job_response(job: &DeployJob) -> DeployJobResponse {
    DeployJobResponse {
        id: job.id.clone(),
        status: job.status.clone(),
        stage: job.stage.clone(),
...
fn command_with_runtime_path(command: &str) -> Command {
    let mut process = Command::new(command);
    if let Some(path_env) = build_runtime_path_env() {
...
fn build_runtime_path_env() -> Option<OsString> {
    ...
    for candidate in runtime_path_candidates() {
...
fn format_command_spawn_error(command: &str, error: &std::io::Error) -> String {
    if error.kind() == std::io::ErrorKind::NotFound {
        return format_command_not_found_message(command);
    }
    error.to_string()
}

fn format_command_not_found_message(command: &str) -> String {
    #[cfg(target_os = "macos")]
    {
        return format!(
            "未找到命令 `{command}`。请确认已安装，并确保桌面应用可访问该命令（已自动尝试 /opt/homebrew/bin 与 /usr/local/bin）。"
        );
    }

    #[cfg(not(target_os = "macos"))]
    {
        format!("未找到命令 `{command}`。请确认已安装并在 PATH 中可用。")
    }
}

async fn run_git_command(args: &[&str], cwd: &FsPath) -> CommandRunResult {
    match command_with_runtime_path("git")
        .args(args)
        .current_dir(cwd)
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        ...
        .await
    {
        ...
        Err(error) => CommandRunResult {
            ok: false,
            stdout: String::new(),
            stderr: format_command_spawn_error("git", &error),
            code: None,
        },
    }
}
...
async fn run_git_command_with_logs(
    ...
    let spawn_result = command_with_runtime_path("git")
        .args(args)
        .current_dir(cwd)
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
    ...
    let mut child = match spawn_result {
        Ok(child) => child,
        Err(error) => {
            let message = format_command_spawn_error("git", &error);
    ...
    let stdout_task = stdout_reader.map(|reader| {
        tokio::spawn(
            async move { read_stream_lines(reader, state_stdout, stdout_job, "stdout").await },
        )
    });
    let stderr_task = stderr_reader.map(|reader| {
        tokio::spawn(
            async move { read_stream_lines(reader, state_stderr, stderr_job, "stderr").await },
        )
    });
...
async fn run_deploy_command_with_logs(
    state: &SharedState,
    job_id: &str,
    command: &str,
    args: &[&str],
    ...
    append_deploy_log(state, job_id, "command", format!("$ {command_text}")).await;

    let spawn_result = command_with_runtime_path(command)
        .args(args)
        .current_dir(cwd)
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
    ...
    let mut child = match spawn_result {
        Ok(child) => child,
        Err(error) => {
            let message = format_command_spawn_error(command, &error);
            append_deploy_log(state, job_id, "error", &message).await;
            return CommandRunResult {
                ok: false,
                stdout: String::new(),
                stderr: message,
    ...
    let stdout_task = stdout_reader.map(|reader| {
        tokio::spawn(async move {
            read_stream_lines_for_deploy(reader, state_stdout, stdout_job, "stdout").await
        })
    });
    let stderr_task = stderr_reader.map(|reader| {
        tokio::spawn(async move {
            read_stream_lines_for_deploy(reader, state_stderr, stderr_job, "stderr").await
        })
    });
    ...
    match status {
        ...
        Err(error) => {
            ...
            message.push_str(&error.to_string());
            append_deploy_log(state, job_id, "error", error.to_string()).await;
            CommandRunResult {
                ok: false,
                stdout,
                stderr: message,
...
async fn read_stream_lines_for_deploy<R>(
    reader: R,
    state: SharedState,
    job_id: String,
    level: &'static str,
    ...
    R: AsyncRead + Unpin,
{
    ...
    while let Ok(Some(line)) = lines.next_line().await {
        let clean_line = line.trim_end_matches('\r').to_string();
        if !clean_line.is_empty() {
            append_deploy_log(&state, &job_id, level, clean_line.clone()).await;
        }
        collected.push_str(&line);
        collected.push('\n');
    }
...
async fn assert_git_repository(repository_path: &FsPath, label: &str) -> Result<(), ApiError> {
...
async fn list_git_conflict_files(repository_path: &FsPath) -> Vec<String> {
    let result =
        run_git_command(&["diff", "--name-only", "--diff-filter=U"], repository_path).await;
    if !result.ok {
        return Vec::new();
    }
...
fn get_current_date_label() -> String {
...
async fn load_path_settings_from_disk(settings_file: &FsPath) -> EditorPathSettings {
  let raw = match fs::read_to_string(settings_file).await {
    Ok(value) => value,
    Err(_) => return default_editor_path_settings(),
  };

  match serde_json::from_str::<EditorPathSettings>(&raw) {
    Ok(parsed) => EditorPathSettings {
      document_project_path: normalize_loaded_repo_path(
        parsed.document_project_path.trim(),
        default_document_project_path,
      ),
      chain_code_repo_path: normalize_loaded_repo_path(
        parsed.chain_code_repo_path.trim(),
        default_chain_repo_path,
      ),
      backup_root_path: parsed.backup_root_path.trim().to_string(),
      hugo_project_path: parsed.hugo_project_path.trim().to_string(),
    },
    Err(_) => default_editor_path_settings(),
  }
}

fn default_editor_path_settings() -> EditorPathSettings {
  EditorPathSettings {
    document_project_path: default_document_project_path(),
    chain_code_repo_path: default_chain_repo_path(),
    backup_root_path: String::new(),
    hugo_project_path: String::new(),
  }
}

fn normalize_loaded_repo_path(value: &str, default_factory: fn() -> String) -> String {
  let trimmed = value.trim();
  if trimmed.is_empty() {
    return default_factory();
  }

  if contains_embedded_windows_drive_path(trimmed) {
    return default_factory();
  }

  let platform_absolute = PathBuf::from(trimmed).is_absolute();

  #[cfg(not(target_os = "windows"))]
  if looks_like_windows_absolute_path(trimmed) || !platform_absolute {
    return default_factory();
  }

  #[cfg(target_os = "windows")]
  if looks_like_unix_absolute_path(trimmed) || !platform_absolute {
    return default_factory();
  }
...
fn default_document_project_path() -> String {
  match current_home_dir() {
    Some(home) => home.join("Desktop").join("document").to_string_lossy().to_string(),
    None => String::new(),
  }
}

fn default_chain_repo_path() -> String {
  match current_home_dir() {
    Some(home) => home
      .join("go")
      .join("src")
      .join("chain-code.github.io")
      .to_string_lossy()
      .to_string(),
    None => String::new(),
  }
...
fn contains_embedded_windows_drive_path(value: &str) -> bool {
  let bytes = value.as_bytes();
  if bytes.len() < 4 {
    return false;
  }
  ...
  for index in 1..(bytes.len() - 2) {
    ...
    let colon = bytes[index + 1];
    let slash = bytes[index + 2];
    if (prev == b'/' || prev == b'\\')
...
async fn save_path_settings_to_disk(
    ...
    let content = serde_json::to_string_pretty(settings).map_err(|error| {
        ApiError::internal(format!("Failed to serialize path settings: {error}"))
    })?;
...
fn to_path_settings_response(settings: &EditorPathSettings) -> PathSettingsResponse {
    if settings.document_project_path.trim().is_empty() {
        return PathSettingsResponse {
            ...
            hugo_project_path: settings.hugo_project_path.trim().to_string(),
...
fn resolve_hugo_project_path(settings: &EditorPathSettings) -> Result<PathBuf, ApiError> {
    let hugo_project_raw = settings.hugo_project_path.trim();
    if hugo_project_raw.is_empty() {
        return Err(ApiError::bad_request(
            "Hugo project path is not configured. Please set it in settings first.",
        ));
    }

    absolutize_path(hugo_project_raw)
}
...
fn collect_backup_items(
    source_root: &FsPath,
) -> Result<(Vec<BackupCopyItem>, u64, u64, u64), ApiError> {
    ...
    for entry_result in WalkDir::new(source_root).follow_links(false) {
        let entry = entry_result
            .map_err(|error| ApiError::internal(format!("Failed to scan path: {error}")))?;
        ...
        let relative_path = entry_path
            ...
            .map_err(|error| {
                ApiError::internal(format!("Failed to resolve relative backup path: {error}"))
            })?
            .to_path_buf();
...
async fn assert_directory_exists(path: &FsPath, label: &str) -> Result<(), ApiError> {
    let metadata = fs::metadata(path)
        .await
        .map_err(|_| ApiError::not_found(format!("{label} does not exist: {}", path.display())))?;
    if !metadata.is_dir() {
...
fn build_section_sync(abs_dir: &FsPath, rel_dir: &str) -> Result<ContentNode, ApiError> {
    ...
    let section_title = as_string(index_meta.get("title").or(index_meta.get("tiltle")))
        .unwrap_or_else(|| {
            if rel_dir.is_empty() {
                "Docs".to_string()
            } else {
